...
function LoginScreen({ onLogin }) {
  ...
  const login = async (role) => {
    ...
    try {
      const d = await apiFetch("/api/auth/login", {
        method: "POST",
        body: JSON.stringify({ identifier: id.trim(), password: pw, role }),
      });
  ...
  const register = async () => {
    ...
    try {
      ...
      setRegErr(e.message || "Registration failed. Please try again.");
    } finally {
      setRegLoading(false);
    }
  };
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 390, margin: "auto", display: "flex", flexDirection: "column", alignItems: "center", padding: showReg ? "16px 0" : "0" }}>
        ...
        <div className="login-card s1" style={{ width: "100%" }}>
          <div style={{ textAlign: "center", marginBottom: showReg ? 14 : 20 }}>
            <div style={{ width: showReg ? 40 : 50, height: showReg ? 40 : 50, background: "rgba(0,200,83,.13)", border: "1px solid rgba(0,200,83,.3)", borderRadius: 13, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto", marginBottom: showReg ? 8 : 11, animation: "glow 3s ease-in-out infinite" }}>
          ...
          {!showReg ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {err && (
                <div style={{ background: "rgba(223,27,65,.1)", border: "1px solid rgba(223,27,65,.2)", borderRadius: 8, padding: "9px 12px", color: "#FF4D6D", fontSize: 12.5, display: "flex", gap: 6, alignItems: "center", animation: "popIn .3s forwards" }}>
                  <I.Alert />{err}
                </div>
              )}
              <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {["citizen", "collector"].map(r => (
                    <button key={r} onClick={() => setReg(d => ({ ...d, role: r, secret_code: "" }))}
                      style={{ flex: 1, padding: "9px", borderRadius: 8, border: `1.5px solid ${reg.role === r ? "#00C853" : "rgba(255,255,255,.1)"}`, background: reg.role === r ? "rgba(0,200,83,.15)" : "rgba(255,255,255,.04)", color: reg.role === r ? "#00E676" : "rgba(255,255,255,.5)", fontWeight: 700, fontSize: 12.5, cursor: "pointer", fontFamily: "inherit", transition: "all .18s", textTransform: "capitalize" }}>
              ...
              {reg.role === "collector" && (
                <div style={{ animation: "popIn .3s cubic-bezier(.23,1,.32,1) forwards" }}>
                  <label className="login-lbl" style={{ color: "#FFB300" }}>🔐 VIP Security Passcode</label>
                  <input className="login-inp" type="password" placeholder="Enter collector passcode" value={reg.secret_code} onChange={e => setReg(r => ({ ...r, secret_code: e.target.value }))} style={{ padding: "9px 12px", borderColor: "rgba(255,179,0,.3)", background: "rgba(255,179,0,.05)" }} />
                  <p style={{ fontSize: 11, color: "rgba(255,179,0,.6)", marginTop: 5 }}>⚠ Collector accounts require authorisation from your supervisor</p>
                </div>
              )}
        ...
        {!showReg && (
          <p className="s2" style={{ textAlign: "center", marginTop: 10, fontSize: 11, color: "rgba(255,255,255,.15)" }}>
...
function CitizenDashboard({ session, onLogout, onPay }) {
  ...
  const fetchData = useCallback(() => {
    ...
    apiFetch("/api/payments/history", {}, token)
      ...
      .catch(e => {
        setFetchErr(e.message || "Failed to load transactions.");
        setTxns([]);
        setBalance(0);
      })
            ...
            {loading ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <table className="tbl">
                  ...
                  <tbody>
                    {txns.map((t, i) => (
                      <tr key={t.id || i} style={{ animation: `slideUp .4s cubic-bezier(.23,1,.32,1) ${i * .06}s both` }}>
                        <td><span style={{ fontFamily: "monospace", fontSize: 12, color: "rgba(0,200,83,.6)" }}>{t.receipt_id || t.id}</span></td>
  ...
  const confirmPay = async () => {
    ...
    try {
      const d = await apiFetch("/api/payments/initialize", {
        method: "POST",
        body: JSON.stringify({ ...form, amount: +form.amount, payment_method: method, otp: code }),
      }, token);
      setShowOtp(false); setSuccess(d);
    } catch (e) {
      setOtpErr(e.message || "Payment failed. Please try again.");
    } finally {
      setOtpLoading(false);
    }
  };
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 10 }}>
              {otp.map((v, i) => (
                <input key={i} ref={otpRefs[i]} type="text" inputMode="numeric" maxLength={1} value={v}
                  onChange={e => handleOtpKey(i, e)} onKeyDown={e => handleOtpKey(i, e)}
                  style={{ width: 44, height: 52, textAlign: "center", fontSize: 22, fontWeight: 800, border: `2px solid ${v ? "#00C853" : "rgba(0,200,83,.2)"}`, borderRadius: 10, background: v ? "rgba(0,200,83,.12)" : "rgba(255,255,255,.04)", color: "white", outline: "none", fontFamily: "monospace", transition: "all .15s", caretColor: "#00E676" }} />
              ))}
            </div>
...
function CollectorDashboard({ session, onLogout }) {
    ...
    apiFetch("/api/admin/transactions", {}, token)
      .then(d => {
        ...
        if (d.stats) setStats(d.stats);
          ...
          const list = d.transactions || d || [];
          setStats({
            total_volume: list.reduce((s, t) => s + (t.amount || 0), 0),
            transaction_count: list.length,
            verified_count: list.filter(t => t.status === "paid" || t.status === "completed").length,
            pending_count: list.filter(t => t.status === "pending").length,
          });
        }
      })
      .catch(e => {
        setTxnErr(e.message || "Failed to load transactions.");
        setTxns([]);
        setStats({ total_volume: 0, transaction_count: 0, verified_count: 0, pending_count: 0 });
      })
  ...
  const verify = async () => {
    ...
    try {
      ...
      setVerErr(e.message || "Receipt not found.");
    } finally {
      setVerLoading(false);
    }
  };
  ...
  const action = async (act) => {
    ...
    try {
      await apiFetch(`/api/admin/transactions/${verResult.id}/${act}`, { method: "POST" }, token);
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <main style={{ maxWidth: 960, margin: "0 auto", padding: "24px 20px 60px" }}>
          ...
          <div className="s0" style={{ marginBottom: 20, display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            ...
            <button className="btn btn-green" onClick={() => { setShowCash(true); setCashOk(null); setCashErr(""); setCash({ citizen_name: "", levy_type: "Transport Levy", amount: "", phone: "" }); }}
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 20px", fontSize: 14 }}>
              <span style={{ fontSize: 18 }}>💵</span> Log Cash Payment
            </button>
          </div>
            ...
            <div>
              <div className="card sL" style={{ marginBottom: 14 }}>
                ...
                <div style={{ padding: "18px 20px" }}>
                  ...
                  {verErr && (
                    <div style={{ background: "rgba(223,27,65,.1)", border: "1px solid rgba(223,27,65,.2)", borderRadius: 8, padding: "9px 12px", color: "#FF4D6D", fontSize: 12.5, marginBottom: 10, display: "flex", gap: 6, alignItems: "center", animation: "popIn .3s forwards" }}>
                      <I.Alert />{verErr}
                    </div>
                  )}
                  <div style={{ display: "flex", gap: 8 }}>
